...
def is_eligible(patient, auxiliary):
    ...
    return _eligible_with_profile(patient, score, profile)


def _eligible_with_profile(patient, score, profile):
    """Reuse per-turn derived values; the eligibility expression is unchanged."""
    if score["total"] is None or score["riskValue"] == 0:
        return False
